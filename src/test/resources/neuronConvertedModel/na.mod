...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: gatesHHrates[*]/fcond, on: Component(id=na type=ionChannelHH), from gatesHHrates; Component(id=m type=gateHHrates)
    fopenHHrates = m_fcond * h_fcond ? multiply applied to all instances of fcond in: <gatesHHrates> ([Component(id=m type=gateHHrates), Component(id=h type=gateHHrates)]) c2 ([Component(id=m type=gateHHrates), Component(id=h type=gateHHrates)]) ? path based
    
    ? DerivedVariable is based on path: gatesHHtauInf[*]/fcond, on: Component(id=na type=ionChannelHH), from gatesHHtauInf; null
    ? Path not present in component, using factor: 1
    
    fopenHHtauInf = 1 
    
    ? DerivedVariable is based on path: gatesHHratesTau[*]/fcond, on: Component(id=na type=ionChannelHH), from gatesHHratesTau; null
    ? Path not present in component, using factor: 1
    
    fopenHHratesTau = 1 
    
    ? DerivedVariable is based on path: gatesHHratesInf[*]/fcond, on: Component(id=na type=ionChannelHH), from gatesHHratesInf; null
    ? Path not present in component, using factor: 1
    
    fopenHHratesInf = 1 
    
    ? DerivedVariable is based on path: gatesHHratesTauInf[*]/fcond, on: Component(id=na type=ionChannelHH), from gatesHHratesTauInf; null
    ? Path not present in component, using factor: 1
